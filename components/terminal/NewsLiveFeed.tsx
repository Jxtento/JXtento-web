...
export function NewsLiveFeed() {
  ...
  useEffect(() => {
    ...
    // Fetch initial historical news data
    fetch(`${apiUrl}/v1/kol-events?category=news`)
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => console.error("Failed to fetch News events", err));
    ...
    function connect() {
      ...
      ws.onopen = () => {
        console.log("News Alerts WS connected from UI");
      ...
      ws.onclose = () => {
        console.log("News Alerts WS closed");
        ws = null;
        if (keepAliveIntervalId) clearInterval(keepAliveIntervalId);
        if (isMounted) {
